...
public:
	...
	void BindDB(DB_BINDER& _binder, int32_t _curIdx, OUT int32_t& _nextCol) {
...
public:
	template<typename DB_BINDER>
	void BindDB(DB_BINDER& _binder, OUT int32_t _curIdx, OUT int32_t& _nextCol) {
...
public:
	...
	void BindDB(DB_BINDER& _binder, OUT int32_t _curIdx, OUT int32_t& _nextCol) {
		...
		memset(greetingMent2, sizeof(greetingMent2), 0);
		_binder.BindCol(_curIdx++, OUT greetingMent2);
		_binder.BindCol(_curIdx++, OUT cDate);
		_binder.BindCol(_curIdx++, OUT mDate);
		_nextCol = _curIdx;
	}
